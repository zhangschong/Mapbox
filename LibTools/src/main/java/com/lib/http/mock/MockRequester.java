...
public class MockRequester implements IHttpRequester {
    ...
    public RequestCall request(Method method, Object callBack, Class cls, Object... objs) {
        MockRequestNode node = MockRequestNode.newRequestNode(cls);
        node.mCallBack = callBack;
        MethodDone.doIt(this, "createData", method, node, callBack, cls, objs);
        return node;
    }

    @MethodTag(threadType = IMethodDone.THREAD_TYPE_THREAD)
    private void createData(Method method, MockRequestNode node, Object callBack, Class cls, Object... objs) throws InstantiationException, IllegalAccessException {
        DefaultResponse response = DefaultResponse.createResponse(method, objs);
        node.mResponse = response;
